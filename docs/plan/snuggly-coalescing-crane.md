...
- `scanPerformanceEntries` result: same as today, with slightly *more* history retained thanks to the 500-entry buffer bump.
...
Behavioral test in `src/__tests__/collectImages.test.ts` (extend existing file):
5. **Images loaded before first scan still appear without observers**. `noopOptions.drainPerfObserverCache` returns `[]`; mock `scanPerformanceEntries` to return URLs representing resources loaded during page load; add a DOM `<img>`; run `collectImages`; expect both sources merged into output.
