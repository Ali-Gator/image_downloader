...
Result: Sentry shows 0 errors in the last 14 days, but the Sentry weekly report says **8.4k events dropped** (Mar 21-28).
...
**Steps**:
- [ ] Replace `'blocked'` filter with narrower patterns:
  - `'net::err_blocked'` — Chrome network-level block
  - `'safe browsing'` — Google Safe Browsing
  - `'blocked by client'` — ad blocker / privacy extension
  - This lets through CORS/CSP/fetch errors that we might need to fix
- [ ] Remove `'unknown error.'` filter entirely
- [ ] Deploy and wait 3-5 days
- [ ] Check Sentry: what new errors appear?
...
**Steps**:
- [ ] **B4 (cannot access contents of the page)**: Instead of throwing, show user a message like "Cannot access this page — try granting permission or open a different page". Suppress the error after showing the message.
- [ ] **B3 (no sw)**: Investigate when SW dies. Add SW registration check + recovery. If unrecoverable, show user a message to reload the extension.
- [ ] Deploy, wait 3-5 days, check Sentry
- [ ] Document findings below

**Hypothesis**: B4 is the biggest volume (3k+/month). If we handle it gracefully in UI, we can remove the filter AND improve UX.
...
**Steps**:
- [ ] **B6+B7 (receiving end / could not establish connection)**: Add retry with health check before sending messages to content script. Use `HEALTH_CHECK` message pattern that already exists in codebase.
- [ ] **B5 (could not load file)**: Detect stale extension state, prompt user to reload. Or use `chrome.runtime.reload()` if in background.
