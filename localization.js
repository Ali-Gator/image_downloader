// Функция для получения локализованной строки по ключу с поддержкой параметров
function getLocalizedMessage(messageName, substitutions = null) {
  return chrome.i18n.getMessage(messageName, substitutions);
}

// Функция для локализации элемента по ID с поддержкой параметров
function localizeElementById(elementId, messageName, substitutions = null) {
  const element = document.getElementById(elementId);
  if (element) {
    const message = getLocalizedMessage(messageName, substitutions);
    if (message) {
